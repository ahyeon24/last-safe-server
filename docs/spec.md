...
**착수 시점 방침**: 통계 집계 로직(`BUS_LAST_TIME_STAT`, 배치)은 Phase 2에서 만들지만, `BUS_ARRIVAL_LOG` 폴링 자체는 관측 데이터 축적에 시간이 걸리므로 MVP 완성 직후 조기 착수한다. 서울시 버스 API 가용 여부 검증(10절) 완료가 선행 조건. 자세한 이유는 `CLAUDE.md` "실제 출시 전 반드시 처리해야 할 것" 참고.
